...
\begin{abstract}
Praca zawiera omówienie bezpieczeństwa telefonów komórkowych wyposażonych w system operacyjny Android. 
Opisane są popularne techniki przeprowadzania ataku oraz mechanizmy przed nimi chroniące.
Przedstawione są wybrane podatności w bibliotece WebKit oraz implementacja ataków, które pozwalają na uzyskanie zdalnego dostępu do urządzenia.
\end{abstract}
...
oraz wykonywania zdjęć fotograficznych. Pamięć tego typu urządzeń bardzo często zawiera wiele ważnych informacji na temat jego właściciela.
Daje to pokusę do poszukiwania błędów w aplikacjach zainstalowanych na telefonie, które pozwolą na przejęcie dostępu danych znajdujących się
na urządzeniu oraz na wykonywanie czynności w imieniu właściciela telefonu.
...
badaną platformę oraz potencjalne wektory ataków, np. komunikacja Wi-Fi, 
Bluetooth, GSM, luki w jądrze systemu lub dostępnych aplikacjach.

W rozdziale \ref{chapter:platforma} omówiony został zarys architektury procesorów ARM, w które jest wyposażonych większość urządzeń przenośnych. 
...
Rozdział \ref{chapter:techniki} przedstawia podstawowe techniki ataków, które umożliwiają wykonanie dostarczonego kodu, czyli przepełnienie bufora
...
Rozdział \ref{chapter:przyklady} zawiera praktyczne przykłady ataków i ich implementację. Opisane zostały wybrane podatności w bibliotece WebKit, 
która jest silnikiem domyślnej przeglądarki internetowej w systemie Android. Przedstawiona jest analiza działania programu, który wczytuje odpowiednio 
spreparowane strony HTML, które pozwalają na zdalne przejęcie kontroli nad telefonem. Opisany jest proces tworzenia \textit{shellcode'u}, czyli 
...
z architekturą procesora danego urządzenia. Takie fragmenty kodu maszynowego nazywane są \textit{shellcode'em}. Aby przekazany kod został wykonany 
...
Sposób, w jaki zostanie to zrobione, jest zależny od błędu w programie. Poniżej zostanie przedstawione wstrzyknięcie kodu z wykorzystaniem nadpisania 
...
umieszczony w miejscu, z~którego biblioteka \verb+libwebcore.so+ w instrukcji 4a578 pobiera wartość rejestru R0. Zostało to oznaczone kolorem oliwkowo 
zielonym. Bloki pamięci utworzone poprzez \textit{heap spray} zawierają dwa rodzaje zawartości. Żółte prostokąty przedstawiają pamięć wypełnioną
...
\verb+parseFloat+ i polega na nie niepoprawnym parsowaniu niestandardowej reprezentacji liczb zmiennoprzecinkowych. Implementacja metody
...
i sygnalizujący. Różnią się one tym, że w cichych nie-liczbach najstarszy bit mantysy jest równy 1, a w sygnalizujących jest on równy 0. Większość
...
najstarszy bit mantysy jest równy 1, a więc powstała w wyniku parsowania wartość NaN jest sygnalizująca. Pozostałe cyfry liczby szesnastkowej mogą 
być ustawione dowolnie. 
...
serwera TCP, który nasłuchuje na odpowiednim porcie. W tym celu można użyć narzędzia \verb+netcat+, np. uruchomić serwer na porcie 4444 można 
...
rejestrów R0~--~R5 i LR. Odłożona wartość rejestru LR zostanie użyta do powrotu z podprogramu. W linii 2  do rejestru R5 kopiowany jest przekazany
w parametrze adres struktury \verb+sockaddr_in+. Następnie w liniach 3~--~5 do rejestrów R0~--~R2 zapisywane są argumenty wywołania funkcji systemowej
